...
function Login() {
  ...
  <ExtraNavbar/>
    ...
    <input type="email" name="email" onChange={handleInput} placeholder='Enter Your Email' className='border-4 p-2 w-60 sm:w-80' />
    {error.email && <span> {error.email}</span>}<br />

    <input type="password"  name="password" onChange={handleInput} placeholder='Enter Your Password' className='border-4 w-60 sm:w-80 p-2'/>
    {error.password && <span >{error.password}</span>}

    <Link className='text-xs ml-20  hover:text-lime-950 hover:font-bold'>Forgot Your Password?</Link><br />


    <button type='submit' className='border border-stone-950 p-2 px-2  my-2 bg-blue-600 text-white rounded-lg'>Log in</button>
